...
MONGO_URL=${MONGO_URL:-"mongodb://mongodb:27017/cyclotron"}
LISTEN_PORT=${LISTEN_PORT:-"8087"}
BASE_PATH=${BASE_PATH:-"/"}
WEB_SERVER=${WEB_SERVER:-"http://cyclotron-site:80"}
...
LDAP_URL=${AUTH_LDAP_URL:-"ldap://ldap.example.com:389"}
LDAP_SEARCH_BASE=${LDAP_SEARCH_BASE:-"ou=users, dc=example, dc=com"}
LDAP_SEARCH_FILTER=${LDAP_SEARCH_FILTER:-"(cn={{username}})"}
LDAP_ADMIN_DN=${LDAP_ADMIN_DN:-"cn=cyclotron, ou=apps, dc=example, dc=com"}
LDAP_ADMIN_PASSWORD=${LDAP_ADMIN_PASSWORD:-""}
LDAP_SEARCH_CATEGORIES=${LDAP_SEARCH_CATEGORIES:-"/* name: '', dn: '' */"}

AUTH_ADMINS=${AUTH_ADMINS:-""}

REQUEST_LIMIT=${REQUEST_LIMIT:-"5mb"}
